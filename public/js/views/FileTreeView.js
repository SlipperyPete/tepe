define([
	...
	var FileTreeView = Backbone.View.extend({
		...
		render: function(){
			this.$el.html("");
			var tr = '&#9660;';
			var rootNode = $('<div class="sidebar-brand" style="padding-left:12px;">' + '<div class="name-info">' + '<span class="toggle-folder" data-fetched="1" data-location="'+this.options.name+'">'+'</span>' + '<span class="node-name" style="font-weight:bold;">' + 'FOLDERS' + '</span>' + '</div>' + '<div class="folders-container">' + '</div>' + '<div class="files-container">' + '</div>' + '</div>');
		...
		buildFileTree: function(el, options, isRootFolder){
			...
			_.each(options.folders.sort(), function(name){
				...
				el.find(".folders-container").first().append('<div class="tree-node" style="padding-left:12px; line-height:30px; font-size:15px;">' + '<div class="name-info">' + '<span class="toggle-folder" data-fetched="0" data-location="'+name+'" title="'+name+'">'+ra+'</span>' + '<span class="node-name">'  + n + '</span>' + '</div>' + '<div class="folders-container" style="display:none;">' + '</div>' + '<div class="files-container" style="display:none;">' + '</div>' + '</div>');
			});
			_.each(options.files.sort(), function(name){
				var names = name.split('/');
				var n = names[names.length-1];
				el.find(".files-container").last().append('<div id="file-'+self.fileIndex+'" class="file-name" style="margin-left:'+'12px" data-location="'+name+'" title="'+name+'">'+n+'</div>');
				self.fileIndex++;
			});
			console.log(options);
		},
		createFolderStructure: function(folder, folderName, wrapper, isRoot, scope){
			...
			var self = this;
			var tr = isRoot?'&#9660;':'&#9654;';
			var rootNode = $('<div class="tree-node">' + '<div class="name-info"><span class="toggle-folder">'+tr+'</span><span class="node-name">' + folderName + '</span></div>' + '<div class="folders-container"></div>' + '<div class="files-container"></div>' + '</div>');
		...
		toggleFolderContent: function(e){
			...
			var self = this;
			var target = $(e.target), dataFetched = false, dataLocation = false;
